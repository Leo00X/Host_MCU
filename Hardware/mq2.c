...
void MQ2_Init(void)
{
    ...
    CKCU_PeripClockConfig(CKCUClock, ENABLE);   //使能外设时钟
		...
		GPIO_DirectionConfig(HT_GPIOA,GPIO_PIN_1,GPIO_DIR_IN); //配置PD1/2/3为输入模式	
//		GPIO_PullResistorConfig(HT_GPIOA,GPIO_PIN_1,GPIO_PR_DOWN); //上拉
    ...
    ADC_RegularGroupConfig(HT_ADC, CONTINUOUS_MODE, 2, 1);  //配置ADC转换模式为连续转换，队列长度为2
...
//	u16  pa1_value = ((HT_ADC->DR[0] & 0x0FFF));
    ...
    u16 pa1_value = ADC_GetConversionData(HT_ADC, ADC_REGULAR_DATA0);
	return pa1_value;
