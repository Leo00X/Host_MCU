...
中央控制主机是整个系统的指挥中心，采用高性能的 `HT32F52352`单片机作为核心（后续计划迁移至`HT32F52367`）。它对内作为Zigbee网络协调器管理所有从机节点；对外通过 `ESP8266` Wi-Fi模块连接 `OneNET`云平台，承担物联网网关的职责；在本地，它驱动HMI触摸屏，实现对系统状态的全面监测与直接控制 。
...
### 设计文档
- 📋 [通信协议规范](./docs/05-设计文档/通信协议/) - Zigbee/HMI协议详细设计
- ⚙️ [硬件设计](./docs/05-设计文档/硬件设计/) - 硬件原理图与选型

### AI 协作
- ⚡ [快速开始](./.agent/workflows/1-01-quick-start.md) - 30秒让 AI 了解项目
- 🤖 [AI 协作指南](./.agent/workflows/1-02-dev-with-ai.md) - 高效协作开发
...
* **本地HMI交互：** 驱动 `淘晶驰T1系列3.5寸串口HMI触摸屏`，通过自主设计的UI界面，实现对全部节点（主机与各个从机）状态的全面监测与直接控制。支持双向通信（接收HMI指令）。
* **中央环境监测：** 自身集成了 `DHT11`、`MQ2`、`MQ5`、`PM2.5`等多种传感器，用于监测中央区域的环境状况，为系统决策提供关键数据。
//暂无* **语音控制：** 集成 `ASR-PRO` 语音识别模块，支持语音指令控制新风系统。
...
| 组件         | 型号/规格                                 | 备注                            |
| :----------- | :---------------------------------------- | :------------------------------ |
| 核心MCU      | `HT32F52352` (BM53A367A 开发板)         | 系统主控 (后续迁移至HT32F52367) |
| Wi-Fi模块    | `ESP8266`                               | 连接OneNET云平台                |
| Zigbee模块   | `E18-MS1-PCB` / `DL-20` (CC2530)        | 用于与从机组网 (计划升级至E72)    |
| HMI触控屏    | 淘晶驰T1系列 3.5寸串口屏 (TJC4832T135011) | 本地人机交互界面                |
| 温湿度传感器 | `DHT11`                                 | 监测主机区域环境                |
| 烟雾传感器   | `MQ2`                                   | 监测主机区域烟雾                |
| 煤气传感器   | `MQ5`                                   | 监测主机区域可燃气体            |
